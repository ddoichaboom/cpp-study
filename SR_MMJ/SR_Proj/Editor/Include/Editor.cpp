                     ...
                     _In_ int       nCmdShow)
{
    ...
    CEditorApp* pEditorApp = CEditorApp::Create(g_hWnd, g_hInst);
    ...
    // CEditorApp 정리 (CMainApp 참고)
    _ulong dwRefCnt(0);
    if (dwRefCnt = Safe_Release(pEditorApp))
    {
        MSG_BOX("CEditorApp Release Failed");
        return -1;
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    // ImGui 메시지 처리 - 윈도우 시스템 메시지 이후에 처리
