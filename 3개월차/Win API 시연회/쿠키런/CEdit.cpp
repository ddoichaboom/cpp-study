...
CEdit::CEdit()
	: m_bTestMode(false), m_bEditMode(false), m_iEditType(EDIT_FIRST_PLATFORM),
	...
	if (CKeyMgr::Get_Instance()->Key_Down(VK_LBUTTON))
	{
		...
		if (m_bEditMode)
		{
			...
			case EDIT_SECOND_PLATFORM:
				...
				if (!bObjectExists)
					CTileMgr::Get_Instance()->Add_Tile(pt.x, pt.y, m_pFrameKey);				// 플랫폼의 FrameKey를 인자로 전달
				break;
...
// POINT screenPt = m_tCursor; // 월드좌표 → 화면좌표
	...
	swprintf_s(szEditState, _countof(szEditState),
		L"편집 모드 (E) : %s  테스트 모드 (P) : %s  수직선 (R) : %s  편집 단계 (I) : %d 장애물 타입 (O) : %d 젤리 타입 (J) :  %d 젤리 글자 타입 (T) : %d",
		EditMode, TestMode, VerticalLineMode, m_iEditType,m_iObstacleType, m_iJellyType, m_iJellyTextType);
